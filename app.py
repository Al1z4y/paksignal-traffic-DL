...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**PCU weights**")
    for cls, w in PCU_WEIGHTS.items():
        st.markdown(f"- `{cls}`: {w}")

    st.divider()
    st.caption("Dataset: density-traffic-controller-v1 v7 (Roboflow)")
...
with col_n:
    ...
    uploads["North"] = st.file_uploader(
        "North feed", type=["jpg", "jpeg", "png", "mp4", "avi", "mpeg4"], key="North"
    )
with col_s:
    st.markdown("### ⬇️ South")
    uploads["South"] = st.file_uploader(
        "South feed", type=["jpg", "jpeg", "png", "mp4", "avi", "mpeg4"], key="South"
    )
with col_e:
    st.markdown("### ➡️ East")
    uploads["East"] = st.file_uploader(
        "East feed", type=["jpg", "jpeg", "png", "mp4", "avi", "mpeg4"], key="East"
    )
with col_w:
    st.markdown("### ⬅️ West")
    uploads["West"] = st.file_uploader(
        "West feed", type=["jpg", "jpeg", "png", "mp4", "avi", "mpeg4"], key="West"
    )
...
def _demo_lane(direction: str) -> dict:
    ...
    return {
        ...
        "preview_type": "demo",
        "preview_bytes": None,
        "preview_name": "Demo mode",
    }


def _get_uploaded_preview(uploaded_file) -> dict:
    """
    Store uploaded file bytes so the dashboard can display the same lane feed
    after YOLO analysis. This avoids uploaded_file.read() consuming the file.
    """
...
def _display_lane_preview(lane: dict):
    """Display image/video preview for a lane result."""
    preview_type = lane.get("preview_type")
    preview_bytes = lane.get("preview_bytes")
    preview_name = lane.get("preview_name", "")

    if preview_type == "image" and preview_bytes:
        try:
            img = Image.open(io.BytesIO(preview_bytes))
            st.image(
                img,
                caption=f"{lane['direction']} feed — {preview_name}",
                use_container_width=True,
            )
        except Exception as exc:
            st.warning(f"Could not display image preview: {exc}")

    elif preview_type == "video" and preview_bytes:
        st.video(preview_bytes)
        st.caption(f"{lane['direction']} video feed — {preview_name}")

    elif lane.get("source_type") == "demo":
        st.info("Demo mode uses simulated counts, so no uploaded feed is shown.")
...
if analyze_clicked:
    ...
    # ── Real mode: run YOLO on uploaded feeds ─────────────────────────────────
        ...
        for i, (direction, uploaded_file) in enumerate(active_feeds.items()):
            ...
            # Keep uploaded file preview for dashboard display
            preview_data = _get_uploaded_preview(uploaded_file)
            file_bytes = preview_data["preview_bytes"]
            ...
            try:
                ...
                # Attach preview info to result so it can be displayed later
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # Section 1: Uploaded lane feed previews
    # ═══════════════════════════════════════════════════════════════════════════
    st.header("🖼️ Uploaded Lane Feeds")
    st.markdown(
        "These are the traffic feeds provided for each direction. "
        "The detection results below are calculated from these uploaded inputs."
    )

    feed_cols = st.columns(len(lane_results))

    for col, lane in zip(feed_cols, lane_results):
        with col:
            st.markdown(f"### {lane['direction']}")
            _display_lane_preview(lane)
            st.metric("Vehicles", lane["total_vehicles"])
            st.metric("PCU", f"{lane['pcu_score']:.1f}")
            st.metric("Green", f"{lane['green_time']} s")

    st.divider()

    # ═══════════════════════════════════════════════════════════════════════════
    # Section 2: Per-lane cards
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f'<p class="disclaimer">{env_impact["note"]}</p>', unsafe_allow_html=True)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # Section 5: Final signal decision
    # ═══════════════════════════════════════════════════════════════════════════
    st.divider()
...
st.markdown(
    ...
    "Perception: YOLOv8n trained on density-traffic-controller-v1 v7 (Roboflow). "
